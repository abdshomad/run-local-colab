...
echo "Logging container output to ${LOG_FILE}"

if docker run --gpus=all -p "${HOST_PORT}:${CONTAINER_PORT}" "${IMAGE}" >"${LOG_FILE}" 2>&1 & then
  DOCKER_PID=$!
  echo "Container started in background (PID ${DOCKER_PID})."
  echo "Follow logs with: tail -f ${LOG_FILE}"
else
  echo "Failed to start docker container. Check ${LOG_FILE} for details." >&2
  exit 1
fi
